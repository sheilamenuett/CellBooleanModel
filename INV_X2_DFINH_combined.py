...
# Trends Index => 0: Vertical | 1: Horizontal | 2: Orthogonal
Edge_Net = exprvars('edge_net', 26, 36, 4, 3, 3, 2)

# ===> Instruction(Create)  2D Routing Style p(e,n,s) <===
# ===> Edge_Net_Subnet_NetID[X, Y, Z, Trends, Masks, Subnets]
# Trends Index => 0: Vertical | 1: Horizontal | 2: Orthogonal
Edge_Net_Subnet1 = exprvars('edge_net_subnet1', 26, 36, 4, 3, 3, 3)
Edge_Net_Subnet2 = exprvars('edge_net_subnet2', 26, 36, 4, 3, 3, 3)
...
MaxZ = 3
#Net = 2
#cellName = INV_X2_DFINH_combined
#Initialize poly
...
for x in range(10,10+1):
  for y in range(24,33+1) :
    outLayout[x][y][0][0] = 1
# Net-1 subNet-2 Terminal[3] to Terminal[1]
# AIL1(10,10,24,33) ==> AIL1(14,14,1,10)   Form(minX, maxX, minY, maxY)
...
Net1_Subnet0_C6 = And(
And(*[And(*[And(*[And(*[Implies(Edge_Net_Subnet1[x,y,z,trend,1,0], And(*[And(~Edge_Net_Subnet1[x,y,z,trend,2,s2])for s2 in range(0+1,3)]))for trend in range(0,1+1)])for x in range(0,10+1)])for y in range(0,35+1)])for z in range(2,3+1)]),
And(*[And(*[And(*[And(*[Implies(Edge_Net_Subnet1[x,y,z,trend,2,0], And(*[And(~Edge_Net_Subnet1[x,y,z,trend,1,s2])for s2 in range(0+1,3)]))for trend in range(0,1+1)])for x in range(0,10+1)])for y in range(0,35+1)])for z in range(2,3+1)]),
).to_cnf()
...
Edge_Net_Subnet1[14:25+1,0:35+1,0:3+1,0:2+1,0:2+1,1]=exprzeros(15552)
...
And(*[And(*[And(*[And(*[Implies(Edge_Net_Subnet1[x,y,z,trend,1,1], And(*[And(~Edge_Net_Subnet1[x,y,z,trend,2,s2])for s2 in range(1+1,3)]))for trend in range(0,1+1)])for x in range(2,14+1)])for y in range(16,35+1)])for z in range(2,3+1)]),
...
And(*[And(*[And(*[And(*[Implies(Edge_Net_Subnet1[x,y,z,trend,2,1], And(*[And(~Edge_Net_Subnet1[x,y,z,trend,1,s2])for s2 in range(1+1,3)]))for trend in range(0,1+1)])for x in range(2,14+1)])for y in range(16,35+1)])for z in range(2,3+1)]),
).to_cnf()
...
Net1_Subnet1_R3 = And(
	And(*[And(*[And(*[And(*[And(*[And(*[Implies(Edge_Net[x,y,z,trend,mask,0],~Edge_Net[x,y,z,trend,mask,net2])for net2 in range(1,1+1)])for x in range(2,14+1)])for y in range(16,35+1)])for z in range(1,3+1)])for trend in range(0,2+1)])for mask in range(0,2+1)]),
	).to_cnf()
...
Edge_Net_Subnet1[18:25+1,0:35+1,0:3+1,0:2+1,0:2+1,2]=exprzeros(10368)
...
### Routability Constraints
Net1_Subnet2_R1 = And(
	RConstraints.R1(Edge_Net_Subnet1,[10,24,0,0, 10,25,0,0, 10,26,0,0, 10,27,0,0, 10,28,0,0, 10,29,0,0, 10,30,0,0, 10,31,0,0, 10,32,0,0, 10,33,0,0, ],10,6,0,1,18,35,3,2),
...
Net1_Subnet2_R2 = And(
	RConstraints.R2_R4_V3(Edge_Net_Subnet1,Edge,Edge_Net,[10,24,0, 10,25,0, 10,26,0, 10,27,0, 10,28,0, 10,29,0, 10,30,0, 10,31,0, 10,32,0, 10,33,0, 10,34,0, 14,1,0, 14,2,0, 14,3,0, 14,4,0, 14,5,0, 14,6,0, 14,7,0, 14,8,0, 14,9,0, 14,10,0, 14,11,0, ],22,6,0,0,18,35,3,2,0),
	)
Net1_Subnet2_R3 = And(
...
# Range R2(12,12,0,35)
### Disable edges outside window
Edge_Net_Subnet2[16:25+1,0:35+1,0:3+1,0:2+1,0:2+1,1]=exprzeros(12960)
...
FORMULA = And(Net1_Subnet0_Formula, Net1_Subnet1_Formula, Net1_Subnet2_Formula, Net2_Subnet1_Formula, )
endTime = time.time()
