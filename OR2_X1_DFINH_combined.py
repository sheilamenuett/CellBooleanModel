...
# Trends Index => 0: Vertical | 1: Horizontal | 2: Orthogonal
Edge_Net = exprvars('edge_net', 26, 36, 4, 3, 3, 4)

# ===> Instruction(Create)  2D Routing Style p(e,n,s) <===
# ===> Edge_Net_Subnet_NetID[X, Y, Z, Trends, Masks, Subnets]
# Trends Index => 0: Vertical | 1: Horizontal | 2: Orthogonal
Edge_Net_Subnet1 = exprvars('edge_net_subnet1', 26, 36, 4, 3, 3, 1)
...
MaxZ = 3
#Net = 4
#cellName = OR2_X1_DFINH_combined
#Initialize poly
...
Edge_Net[14,11,0,0,1,0:] = exprzeros(4)
Edge_Net[14,12,0,0,1,0:] = exprzeros(4)
#(R)End initialize EdgeNet
#(L)Disable GIL
Edge[10,0,1,1,1] = expr(0)
Edge[10,1,1,1,1] = expr(0)
...
Edge_Net[14,22,0,0,1,0:] = exprzeros(4)
Edge_Net[14,23,0,0,1,0:] = exprzeros(4)
...
Edge_Net_Subnet1[6:25+1,0:35+1,0:3+1,0:2+1,0:2+1,0]=exprzeros(25920)
...
Net1_Subnet0_R3 = And(
	And(*[And(*[And(*[And(*[And(*[And(*[Implies(Edge_Net[x,y,z,trend,mask,0],~Edge_Net[x,y,z,trend,mask,net2])for net2 in range(1,3+1)])for x in range(0,6+1)])for y in range(0,35+1)])for z in range(1,3+1)])for trend in range(0,2+1)])for mask in range(0,2+1)]),
	).to_cnf()
...
### Disable edges outside window
Edge_Net_Subnet2[14:25+1,0:35+1,0:3+1,0:2+1,0:2+1,1]=exprzeros(15552)
...
Net2_Subnet1_R3 = And(
	...
	And(*[And(*[And(*[And(*[And(*[And(*[Implies(Edge_Net[x,y,z,trend,mask,1],~Edge_Net[x,y,z,trend,mask,net2])for net2 in range(2,3+1)])for x in range(0,14+1)])for y in range(0,35+1)])for z in range(1,3+1)])for trend in range(0,2+1)])for mask in range(0,2+1)])	).to_cnf()
...
Edge_Net_Subnet2[18:25+1,0:35+1,0:3+1,0:2+1,0:2+1,2]=exprzeros(10368)
...
FORMULA = And(Net1_Subnet0_Formula, Net2_Subnet1_Formula, Net2_Subnet2_Formula, )
endTime = time.time()
